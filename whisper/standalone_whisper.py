...
try:
    ...
    print("Warning: samplerate not available, using scipy for resampling")
import torch
import whisper
...
class StandaloneWhisperApp:
    ...
    def _load_or_create_mel_filter(self):
        ...
        try:
            ...
            mel_filter_path = os.path.join(script_dir, "mel_filters.npz")
    ...
    def _chunk_and_resample_audio(
        ...
        if audio_sample_rate != self.sample_rate:
            if HAS_SAMPLERATE:
                ...
                target_length = int(len(audio) * self.sample_rate / audio_sample_rate)
